...
import datetime
import csv
from .resources import AllSparePartResources
...
def newupload(request):
    if request.method == 'POST':
        try:
            AllSparePart_resource = AllSparePartResources()
            dataset = Dataset()
            new_sp = request.FILES['myfile']
            if not new_sp.name.endswith('xlsx'):
                messages.info(request,'wrong format')
                return render(request,'home/newupload.html')

            import_data = dataset.load(new_sp.read(),format='xlsx')
            for data in import_data:
                value = AllSparePart.objects.create(
                    user=request.user,
                    partCode=data[2],
                    goodStatus=data[3],
                    sparePartType=data[4],
                    csceHWMO=data[5],
                    serialNumber=data[6],
            ...
            return redirect('uploadfromfileerror')
    context = {}
    return render(request,'home/newupload.html',context)
